...
import faust.faustmod;
import faust.util.CardStats;
import faust.util.TriFunction;
...
import static faust.util.GeneralUtils.removePrefix;
import static faust.util.TextureLoader.getCardTextureString;


public abstract class BaseCard extends CustomCard {
    final private static Map<String, DynamicVariable> customVars = new HashMap<>();

    protected static String makeID(String name) { return faustmod.makeID(name); }
    ...
    public void upgrade()
    {
        if (!upgraded)
        {
            ...
            if (this.upgradesDescription)
            {
                ...
                {
                    faustmod.logger.error("Card " + cardID + " upgrades description and has null upgrade description.");
                }
                else
                {
